...
const populateCourses = async (data) => {
  ...
  for (let i = 0; i < data.prereqs.length; i++) {
    try {
      ...
      prereqList.push(<li key={d[0].id}><a href= {'/courses/' + d[0].id} > { d[0].course_name } </a></li>);
    } catch (err) {
      console.log(err);
    }
  }
  ...
  for (let i = 0; i < data.coreqs.length; i++) {
    try {
      ...
      coreqList.push(<li key={d[0].id}><a href= {'/courses/' + d[0].id} > { d[0].course_name } </a></li>);
    } catch (err) {
      console.log(err);
    }
  }
  ...
  for (let i = 0; i < data.followups.length; i++) {
    try {
      ...
      followupList.push(<li key={d[0].id}><a href= {'/courses/' + d[0].id} > { d[0].course_name } </a></li>);
    } catch (err) {
      console.log(err);
    }
  }
...
const populateRelationships = async (data) => {
  ...
  for (let i = 0; i < data.skills.length; i++) {
    try {
      ...
      const d = await resp.json();
      console.log(d);
      skillsList.push(<li key={d[0].id}><a href= {'/skills/' + d[0].id} > { d[0].skill_name } </a></li>);
    } catch (err) {
      console.log(err);
    }
  }
